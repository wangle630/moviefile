...
exports.add = function(db, req, res) {
    exports.parseReceivedData(req, function(work) {  //解析HTTP POST数据
        db.query(
            "INSERT INTO t_work (hours, date, description) " +
            " VALUES (?, ?, ?)",  //添加工作记录的SQL
...
exports.delete = function(db, req, res) {
    exports.parseReceivedData(req, function(work) { //解析HTTP POST数据
        db.query(
            "DELETE FROM t_work WHERE id=?",  //删除工作记录的SQL
            [work.id],  //工作记录ID
            function(err) {
                if (err) throw err;
                exports.show(db, res);  //给用户显示工作记录清单
...
exports.archive = function(db, req, res) {
    exports.parseReceivedData(req, function(work) {  //解析HTTP POST数据
        db.query(
            "UPDATE t_work SET archived=1 WHERE id=?",  //更新工作记录的SQL
            [work.id],  //工作记录ID
            function(err) {
                if (err) throw err;
                exports.show(db, res);  //给用户显示工作记录清单
...
exports.show = function(db, res, showArchived) {
    var query = "SELECT * FROM t_work " +  //获取工作记录的SQL
          "WHERE archived=? " +
    "ORDER BY date DESC";
